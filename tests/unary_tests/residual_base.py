...
class SGTE91UnaryResidual(ResidualFunction):
    ...
    def __init__(
        ...
        weight: Optional[Dict[str, float]]
            ...
            self.data_json = datasets
            # populate self.data from datasets,database
            self.data = {k:{} for k in ["Cp","H","Hfus","Tm"]}
            for jf in self.data_json:
                jsonf = open(jf)
                json_data=json.load(jsonf)
                self.data[json_data['type']].update({json_data['name']:json_data['data']})
                jsonf.close()

    def get_all_unary_data(self):
        """
        BUILDS A DATABASE
        should be replaced by a TinyDB
        opens a series of json files and 
        generates a nested dict
