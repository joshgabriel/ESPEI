...
from espei.error_functions import get_zpf_data, get_thermochemical_data, get_equilibrium_thermochemical_data, load_unary_datasets
...
def setup_context(dbf, datasets, symbols_to_fit=None, data_weights=None, phase_models=None, make_callables=True,unary_path=None,unary_params=None):
    """
    ...
    #if unary_path:
    #   unary_data = load_unary_datasets(glob(unary_path+os.sep+"*.json"))
    ...
    if unary_path:
       unary_data = load_unary_datasets(glob(unary_path+os.sep+"*.json"))
       error_context.update({'unary_kwargs': {
            'unary_data': unary_data,
            'param_spec': unary_params,
            'dbf': dbf
        }})
